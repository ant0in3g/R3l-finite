...
\end{equation}
%
where $S_4$ can be read-off from \eqref{critical-action}. The expansion of both the logarithm and $e^{-S^{\Omega=1}_4}$ then gives rise to the perturbative expansion. Performing a Legendre transformation on \eqref{connected-funct} yields the effective action $\Gamma(\Phi)$, namely%
%
\begin{equation}
\Gamma(\Phi) + W(J) = \tr\left( \Phi_\alpha J_\beta \right) \ , \qquad (\Phi_\beta)^j_{mn} = \frac{\delta W(J)}{\delta(J_\beta)^j_{nm}} \ , \label{legendre}
\end{equation}
%
...
%
Z_j^{\Omega=\frac{1}{3}} &=& \int \mathcal{D}\Phi^j \ \mathcal{D}\Phi^{\dag j} \ \mbox{exp}\left( - \frac{16 \pi \lambda^3}{g^2} w(j) \mbox{tr}_j\left(\Phi^j Q_{\Omega=\frac{1}{3}}^j\Phi^{\dag j}+\Phi^{\dag j} Q_{\Omega=\frac{1}{3}}^j\Phi^j\right) \right) \nonumber \\
&& \times \ \mbox{exp}\left( - \frac{512 \pi \lambda^3}{g^2} w(j) \mbox{tr}_j\left(\Phi^j\Phi^{\dag j}\Phi^j\Phi^{\dag j}\right) \right) \ , \label{zqj} 
\end{eqnarray}
%
where%
%
...
%
\begin{eqnarray}
Z_j^{\Omega=\frac{1}{3}} &=& \int [D\mathsf{U}] \ [D\mathsf{V}] \prod_{k=-j}^j \int_0^{+\infty} dt^j_k \ \left(\Delta\left(\left(R^{j}\right)^2\right)\right)^2 \ e^{-\frac{512 \pi \lambda^3}{g^2} w(j) \mbox{ tr}_j\left(\left(R^{j}\right)^4\right)}  \nonumber \\
&& \times \ e^{-\frac{16 \pi \lambda^3}{g^2} w(j) \mbox{ tr}_j\left(Q_{\Omega=\frac{1}{3}}^j \mathsf{V}^\dag \left(R^{j}\right)^2 \mathsf{V} \ + \ Q_{\Omega=\frac{1}{3}}^j \mathsf{U}^\dag \left(R^{j}\right)^2 \mathsf{U} \right)}  \ , \label{zj-interm}
\end{eqnarray}
%
...
%
\begin{eqnarray}
Z_j^{\Omega=\frac13} &=& N^j \prod_{k=-j}^j \int_0^\infty dt^j_k \ \left( \det_{-j \le p,l \le j} \left(e^{-z t^j_p \omega^j_{l} } \right) \right)^2 \ e^{-32z \left( \overset{j}{\underset{m=-j}{\sum}} \ \left(t^{j}_m\right)^2\right)} \ , \nonumber \\
%
&=& N^j \prod_{k=-j}^j \int_0^\infty  dt^j_k \left( \sum_{\sigma\in\mathfrak{S}_{2j+1}} \left|\sigma\right| \prod_{k=-j}^j \ e^{-zt^j_k\omega^j_{\sigma(k)}} \right)^2 \ e^{-32z\left( \overset{j}{\underset{m=-j}{\sum}} \ \left(t^{j}_m\right)^2\right)} \ , \label{zj-partialbis} 
\end{eqnarray}
%
where%
%
\begin{equation}
z = \frac{16\pi\lambda^3}{g^2} w(j) \ , \quad \mbox{and} \quad N^j = \left(\Delta\left(Q_{\Omega=\frac13}^j\right)\right)^{-2}\left(\prod_{k=1}^{2j}k!\right)^2 z^{-2j(2j+1)} \ . \label{normal-zj}
\end{equation}
%
...
%
\begin{equation}
Z_j^{\Omega=\frac13} = N^j \ \sum_{\sigma_1,\sigma_2\in\mathfrak{S}_{2j+1}} \left|\sigma_1\right| \ \left| \sigma_2 \right| \prod_{k=-j}^j \int_0^\infty dt^j_k \ e^{- 32 z \left(t^j_k\right)^2 - z \omega^j_{\sigma_1\sigma_2(k)} t^j_k } \ , \label{calcul1} 
\end{equation}
%
